# Módulo de Prevenção a Fraude e Rate Limiting (SFCC)

## 1. Introdução
Este documento detalha a implementação de uma solução de segurança para o Salesforce Commerce Cloud (SFCC) visando mitigar fraudes e ataques de força bruta (bots). A solução utiliza uma abordagem híbrida e em camadas:

1.  **Monitoramento de Velocidade (Rate Limiting):** Monitora requisições por IP em janelas de tempo.
2.  **Step-up Authentication:** Solicita reCAPTCHA se o usuário exceder um limite leve (Soft Limit).
3.  **Bloqueio Automático:** Bloqueia o IP temporariamente (15 min) se exceder o limite rígido (Hard Limit).
4.  **Bloqueio Lógico:** Permite bloquear usuários específicos (via CustomerID) caso o gateway de pagamento confirme fraude.
5.  **Gestão:** Painel no Business Manager para visualização e desbloqueio manual.

---

## 2. Arquitetura

Abaixo está o diagrama de componentes seguindo o modelo C4 (Container Diagram), ilustrando a interação entre os Controllers, Scripts Helpers, Custom Objects e Serviços Externos.
...
Rel(vel_mgr, co_velocity, "Lê/Escreve", "Transaction")
...
2. Crie uma Credencial:
   ...
   * **User/Pass:** Deixe em branco (não é necessário para esta API).
...
### C. Site Preferences
1. Crie um grupo ou adicione às preferências existentes.
...
if (vCheck.status === 'REQUIRE_CAPTCHA') {
    ...
    if (!RecaptchaService.verify(captchaToken, secret)) {
        res.json({ error: true, errorMessage: "Falha na validação do Captcha." });
        return next();
    }
...
// 4. Pós-Pagamento (Se houver fraude confirmada pelo Gateway)
...
<div style="padding: 20px; font-family: sans-serif;">
    ...
    <table width="100%" border="1" cellpadding="5" style="border-collapse: collapse;">
        <thead><tr><th>Tipo</th><th>Valor</th><th>Motivo</th><th>Expira em</th><th>Ação</th></tr></thead>
        <tbody>
